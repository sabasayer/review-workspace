...
6. **Be honest about verification.** Mark a Verification item `unverified` unless real proof (a passing test, an observed run, a matching snapshot) already exists. A screenshot or a green mocked test is not proof of real backend behavior. A `gap` naming a concrete concern needs `targetIds` resolving to an Annotation (or an Evidence item) anchored at the exact place — an Annotation that only mentions it in passing, or a Behavioral Group id, doesn't count and sends the reviewer nowhere; the engine flags this as `unanchored-verification-gap`. Only set `diffuse: true` when the concern genuinely isn't about one place (a missing dashboard, an unrunnable pipeline) — never as a shortcut for skipping the anchor.
